# tss_Purgelog.ps1
# Last edit: [we] 2023.09.29

# Local variables
$Script:TSSRegKey		= "HKLM:Software\Microsoft\CESDiagnosticTools\TSS"
$Script:TSSParamRegKey = "$TSSRegKey\Parameters"
$Script:TSSParamRegKeyArray = @{}
...
Function LogMessage{
	...
	If($Level -eq $LogLevel.Warning -or $Level -eq $LogLevel.Error -or $Level -eq $LogLevel.Debug -or $Level -eq $LogLevel.ErrorLogFileOnly -or $Level -eq $LogLevel.WarnLogFileOnly -or $Level -eq $LogLevel.InfoLogFileOnly){
		...
		If($CallerInfo.ScriptName -notlike "*$global:ScriptName"){ # ScriptName = 'TSS.ps1'
			$FuncName = (((Split-path $CallerInfo.ScriptName -leaf) -replace "TSS_","") + ":" + $FuncName) #we# 2023.07.05
		}
...
Function ReadParameterFromTSSReg{
	LogInfoFile "$Script:TSSParamRegKey"
	If(!(Test-Path $Script:TSSParamRegKey)){
		LogInfoFile "There are no parameter settings in TSS registry."
		Return
	}Else{
		LogInfoFile "Reading parameters from TSS registry."
		
		$ParamArray = Get-Item "$Script:TSSParamRegKey" | Select-Object -ExpandProperty Property -ErrorAction Ignore
		$RegValue = Get-ItemProperty -Path  "$Script:TSSParamRegKey" -ErrorAction Ignore
		ForEach($Param in $ParamArray){
			...
			If(!($Script:TSSParamRegKeyArray.ContainsKey($Param))){
				$Script:TSSParamRegKeyArray.Add($Param,$Data)
			}
		}
	}
}
...
###
LogInfoFile "==================== START ===================="
# Read parameters from TSS registry to get log folder and NrFilesToKeep.
ReadParameterFromTSSReg

# Purge ETL files
# Get ETLNumberToKeep and log folder.
If($Script:TSSParamRegKeyArray.ContainsKey('LogFolder')){
    $TSSLogFolder = $Script:TSSParamRegKeyArray['LogFolder']
    If([String]::IsNullOrEmpty($TSSLogFolder)){
    ...
    If([String]::IsNullOrEmpty($TSSLogFolder)){
        LogErrorFile "LogFolder is not set in $($Script:TSSParamRegKey)."
        LogInfoFile "==================== END ===================="
        Return
    }
}

If($Script:TSSParamRegKeyArray.ContainsKey('EtlOptions')){
    $EtlOptions = $Script:TSSParamRegKeyArray['EtlOptions'] -split ':'
...
LogInfoFile "LogFolder = $TSSLogFolder"
$ETLFiles = @()
$AllETLFiles = Get-Item "$TSSLogFolder\*"
...
ForEach($Key in $ETLFileCountByProvider.Keys){
    ...
    If($FileCount -gt $ETLNumberToKeep){
        $CadidateFiles = Get-Item "$TSSLogFolder\*_$Key*.etl" | Sort-Object -Property LastWriteTime
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCPoqKOePAbQocN
# QPmy9wOX+KRE+y3Jm6KE1WPD1VViYqCCDYUwggYDMIID66ADAgECAhMzAAADri01
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIHMJ
# YHBJmizupj2fWysQiOOAgQ3XeOjWivGnn4EwSzctMEIGCisGAQQBgjcCAQwxNDAy
# oBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5j
# b20wDQYJKoZIhvcNAQEBBQAEggEAtLXo7J1ZuaAC9FrqWybwDn/NCFW5/h1kCvsL
# nrIjHG174QBumTavVf2eehc5j57g8f1CXRhtp7qTWu6Shr1hTA/YCIqn6/rZTZUv
# 1xJ0y2tguQBjK71xM6hA7iT+RaTVczutZHoG0OfCEhWyZxJ81Br3KUX+J6eSx7rq
# 7henKD6cZAtIZTGg0DVMjelm0jV6kGINVUUysMqX7l1uVYVX+QZDHamgdPN9hV/R
# x37bdij6eCcpe8Vj7tOldmMAxy6PgvjU/Cn4q0xXBcv7DNPbkN1hEy6Im81lcNI/
# eNxRmwY1P9NFxc8QRRhk2Wh68SajP4CMhOE4dcGjC3UUIBm9NKGCF5QwgheQBgor
# BgEEAYI3AwMBMYIXgDCCF3wGCSqGSIb3DQEHAqCCF20wghdpAgEDMQ8wDQYJYIZI
